...
const postPokemon = async (req,res)=>{
    ...
    if(oldPokemonDB){ return res.status(400).send("El pokemon que estas creando ya existe en la base de datos")}

    try {
        const oldPokemonLink = await axios.get(`${URL}/${name}`)
        if(oldPokemonLink){return res.status(400).send("El pokemon que estas creando ya existe en la API")}        
    } catch (error) {
